...
class BaseStatIndexCd(models.Model):
    """时段索引代码表"""
    start_tm = models.CharField(max_length=8,blank=True, null=True,verbose_name="开始时间")
    end_tm = models.CharField(max_length=8,blank=True, null=True,verbose_name="结束时间")
    stat_end_tm = models.CharField(max_length=8,blank=True, null=True,verbose_name="统计结束时间")
    belong_fif_min_index_cd = models.CharField(max_length=8,blank=True, null=True,verbose_name="归属十五分钟索引代码")
    class Meta:
        db_table = "base_stat_index_cd"

class FlowInOutDetail(models.Model):
    """进出站客流表"""
    ...
    #
    stat_tm = models.CharField(max_length=20,blank=True, null=True, verbose_name="统计时间")
    ...
    scheme_id = models.CharField(max_length=50, null=True, blank=False, verbose_name='仿真方案id')

    class Meta:
        db_table = 'flow_inout_detail'

class FlowTranDetail(models.Model):
    """换乘客流表"""
    station_id = models.CharField(max_length=4, blank=True, null=True,verbose_name="换乘车站编码")
    # 2020-07-01 14:00:00
    stat_tm = models.CharField(max_length=20, blank=True, null=True, verbose_name="统计时间")

    from_line_id = models.CharField(max_length=2,blank=True, null=True, verbose_name="换乘前的线路编号")
    to_line_id = models.CharField(max_length=2,blank=True, null=True, verbose_name="换乘后的线路编号")

    # 01-上行 02-下行
    from_drct_cd = models.CharField(max_length=2,blank=True, null=True,verbose_name="进站列车的行驶方向")
    # 01-上行 02-下行
    to_drct_cd = models.CharField(max_length=2,blank=True, null=True,verbose_name="出站列车行驶方向")
    quantity = models.IntegerField(blank=True, null=True,verbose_name='换乘客流数量')
    # 外键
    scheme_id = models.CharField(max_length=50, null=True, blank=False, verbose_name='仿真方案id')

    class Meta:
        db_table = 'flow_tran_detail'
...
class TrainData(models.Model):
    ...
    arrive_time = models.CharField(max_length=20,blank=True, null=True,verbose_name='到站时间')
    depart_time = models.CharField(max_length=20,blank=True, null=True,verbose_name='离站时间')
...
class EmuPsEquipment(models.Model):
    """乘客设备使用基础表"""
...
class EmuPsFacilities(models.Model):
    """乘客设施使用表"""
    is_transfer = models.CharField(max_length=10, blank=True, null=True, verbose_name="是否为换乘站")
    station_id = models.CharField(max_length=4, blank=True, null=True, verbose_name="车站编号")
    ps_pare_type = models.CharField(max_length=20, verbose_name="乘客参数类型")
...
class EmuInOutRate(models.Model):
    """出入口进出站比例表"""
    station_id = models.CharField(max_length=4, blank=True, null=True, verbose_name="车站编号")
    ps_pare_type = models.CharField(max_length=20, verbose_name="乘客参数类型")
    # F0521010100B
...
class EmuTrainPartRate(models.Model):
    """站台上下车位置比例表"""
    station_id = models.CharField(max_length=4, blank=True, null=True, verbose_name="车站编号")
    ps_pare_type = models.CharField(max_length=20, verbose_name="乘客参数类型")
...
class EmuFenceStatus(BaseModel):
    """导流栏状态表"""
...
class EmuGateWayStatus(BaseModel):
    """仿真出入口状态表"""
